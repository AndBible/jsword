    ...
    <html dir="{$direction}">
      <head>
        ...
        <style type="text/css">
          ...
          A.strongs { color: red; text-decoration: none; }
          A.morph { color: blue; text-decoration: none; }
...
<!--      <xsl:when test="$protocol = $lex.def.protocol">
      ...
      <xsl:when test="$separator = ''">
        <xsl:choose>
          <xsl:when test="$protocol = $lex.def.protocol">
            <sub class="lemma"><xsl:value-of select="$orig-lemma"/></sub>
          </xsl:when>
          <xsl:otherwise>
            ...
            <sub class="strongs"><a class="strongs" href="{$protocol}{$orig-lemma}"><xsl:value-of select="format-number(substring($orig-lemma,2),'#')"/></a></sub>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="$protocol = $lex.def.protocol">
            <sub class="lemma"><xsl:value-of select="substring-before($orig-lemma, $separator)"/>, </sub>
          </xsl:when>
          <xsl:otherwise>
            ...
            <sub class="strongs"><a class="strongs" href="{$protocol}{substring-before($orig-lemma, $separator)}"><xsl:value-of select="format-number(substring(substring-before($orig-lemma, $separator),2),'#')"/></a>, </sub>
          </xsl:otherwise>
        </xsl:choose>
        <xsl:call-template name="lemma">
